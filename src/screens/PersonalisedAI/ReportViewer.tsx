...
import {Image, ScrollView, View} from 'moti';
    ...
    <ScrollView>
      ...
      <View
        ...
        <CustomText className="font-isidoraMedium text-sm text-yankeesBlue mt-2">
          {languages?.generating_health_report}
        </CustomText>
...
const ReportViewer = ({
  ...
  const {data: aiReportDetails, isError} = useGetAIReportDetails({
    token: token_id,
    gcTime: 0,
    staleTime: 0,
    initialData: [],
  ...
  useEffect(() => {
    if (isError) {
      showAlert({
        ...
        content: languages?.ai_report_pdf_fetch_failed,
      });
    }
  ...
  const details = aiReportDetails?.[0];
        ...
        <View
          ...
          <View className="flex-row items-center justify-center px-4">
            ...
            <View className="absolute w-full flex-row justify-end space-x-4">
              <TouchableOpacity
                onPress={() => downloadFile(details?.report_link, 'ai_report')}>
                <Icon
                  name="download"
                  size={20}
