...
const VoiceScanIntro = () => {
  ...
  const ENTRIES1 = [
    {
      title: languages?.voice_scan_carousel_1_title,
      subtitle: languages?.voice_scan_carousel_1_description,
      illustration: VoiceScanCarousel1,
    },
    {
      title: languages?.voice_scan_carousel_2_title,
      subtitle: languages?.voice_scan_carousel_2_description,
      illustration: VoiceScanCarousel2,
    },
    {
      title: languages?.voice_scan_carousel_3_title,
      subtitle: languages?.voice_scan_carousel_3_description,
      illustration: VoiceScanCarousel3,
    },
  ];
  ...
  const navigateToVoiceScan = async () => {
  ...
  const renderItem = ({item}: {item: any}) => (
    <EtchedGlass
      ...
      <ScrollView className="max-h-full" showsVerticalScrollIndicator={false}>
        ...
        <CustomText className="mt-10 text-center text-sm mediumPhone:text-base largePhone:text-base font-isidoraSemiBold">
          {item.title}
        </CustomText>
        <CustomText className="mt-2 text-center text-xs mediumPhone:text-sm largePhone:text-base">
          {item.subtitle}
        </CustomText>
      </ScrollView>
    </EtchedGlass>
    ...
    <BackgroundImage>
      <SafeAreaView className="h-full">
        ...
        <View className="px-10 mb-2">
          <RoundedButton
            onPress={
              isLastSlide ? navigateToVoiceScan : () => ref?.current?.next()
            }>
